...
if(! is_hiseq_dir(prj_dir, "rx")) {
  stop(paste0("Input file is not a HiSeq rx directory: ", prj_dir))
}
```
...
p1 <- plot_hiseq_align(pd$align, 
                       ...
                       add_label = FALSE)
print(p1)
p2 <- plot_hiseq_peak(prj_dir, hiseq_type = TRUE)
print(p2)
...
tss_list <- purrr::keep(tss_list, file.exists) # filter
tss_list <- purrr::keep(
  tss_list,
  function(i) {
    grepl(".vs.", i)
...
# tss_list <- tss_list[grepl(".vs.", basename(dirname(dirname(tss_list))))]
...
# gb_list <- gb_list[grepl(".vs.", basename(dirname(dirname(gb_list))))]
