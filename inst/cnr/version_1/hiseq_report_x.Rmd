...
rep_list      <- purrr::keep(d1, is_hiseq_single_dir)
merge_list    <- purrr::keep(d1, is_hiseq_merge_dir)
multiple_list <- purrr::keep(d1, is_hiseq_multiple_dir)
report_html_list <- lapply(multiple_list, list_hiseq_report) %>% unlist %>% unique

## peaks for merge
df_trim_stat    <- lapply(merge_list, read_hiseq_trim_stat) %>% dplyr::bind_rows()
df_align_stat   <- lapply(merge_list, read_hiseq_align_stat) %>% dplyr::bind_rows()
