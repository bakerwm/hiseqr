...
if(! endsWith(hiseq_type, "_from_design")) {
  stop(paste0("Input file is not a multiple HiSeq directory, _design: ", input_dir))
}
```
...
# list hiseq_r1 dirs 
d1 <- list.dirs(input_dir, recursive = FALSE)
a1 <- unlist(sapply(d1, is_hiseq_single_dir))
a2 <- unlist(sapply(d1, is_hiseq_merge_dir))
a3 <- unlist(sapply(d1, is_hiseq_multiple_dir))

rep_list      <- a1[a1] %>% names
merge_list    <- a2[a2] %>% names
multiple_list <- a3[a3] %>% names ## possible

df_report_list <- sapply(multiple_list, list_hiseq_report) %>% unlist %>% unique()

## peaks for merge
df_trim_stat    <- lapply(merge_list, read_hiseq_trim) %>% dplyr::bind_rows()
df_align_stat   <- lapply(merge_list, read_hiseq_align) %>% dplyr::bind_rows()
df_peak_stat    <- lapply(merge_list, read_hiseq_peak_stat) %>% dplyr::bind_rows()
df_frip_stat    <- lapply(merge_list, read_hiseq_frip_stat) %>% dplyr::bind_rows()
df_lendist_stat <- lapply(merge_list, read_hiseq_lendist_stat) %>% dplyr::bind_rows()

## enrich
png_list <- lapply(multiple_list, function(i){
  data.frame(
    tss = hiseqr::read_hiseq(i)$args$tss_enrich_png,
    genebody = hiseqr::read_hiseq(i)$args$genebody_enrich_png,
    fingerprint = hiseqr::read_hiseq(i)$args$bam_fingerprint
  )
}) %>%
  dplyr::bind_rows()
...
mito     <- df_align_stat %>% pull(chrM) %>% mean() %>% round(digits = 0)
mito_pct <- df_align_stat %>% pull(chrm_pct) %>% mean() %>% round(digits = 0)
...
```{r figure2_peaks-2, out.width = "100%"}
# peaks
df_peak_stat %>%
  dplyr::mutate(id = forcats::fct_rev(id)) %>%
  hiseqr::barplot2(label = TRUE) +
...
## tss
f4 <- png_list$tss
f4 <- f4[file.exists(f4)]
knitr::include_graphics(f4)
```


```{r figure5_genebody_enrich, eval = T, fig.height = 4}
f5 <- png_list$genebody
f5 <- f5[file.exists(f5)]
...
f8 <- png_list$fingerprint
f8 <- f8[file.exists(f8)]
knitr::include_graphics(f8)
```
...
```{r}
# rpt_html <- list_hiseq_report(input_dir)
rpt_html <- df_report_list
