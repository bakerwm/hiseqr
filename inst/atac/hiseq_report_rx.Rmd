...
```{r figure1_trim_reads, fig.height=fig_height, fig.width=6}
...
```{r figure1_align_reads, fig.height=fig_height, fig.width=6}
...
```{r figure1_unique_reads, fig.height=fig_height, fig.width=6}
...
```{r pcr_dup_reads, fig.height=fig_height, fig.width=6}
...
```{r figure2_peaks, fig.height=fig_height, fig.width=8}
...
```{r frip_plot, fig.height=fig_height, fig.width=6}
...
```{r figure3_fragment_length, fig.height=fig_height, fig.width=8}
...
dn <- list_hiseq_dir(input_dir, "_rn") 
tss_list <- list_hiseq_file(dn, "tss_enrich_png", "_rn")
...
dn <- list_hiseq_dir(input_dir, "_rn") 
tss_list <- list_hiseq_file(dn, "genebody_enrich_png", "_rn")
tss_list <- purrr::keep(tss_list, file.exists)
if(length(tss_list) > 0) {
  knitr::include_graphics(unlist(tss_list))
}
...
dn <- list_hiseq_dir(input_dir, "_rn")
cor_list <- list_hiseq_file(dn, "bam_cor_heatmap_png", "_rn")
cor_list <- purrr::keep(cor_list, file.exists)
if(length(cor_list) > 0) {
  knitr::include_graphics(unlist(cor_list))
}
```
...
```{r fig7_bam_pca, eval = TRUE, fig.width=5, fig.height=5, eval = F}
dn <- list_hiseq_dir(input_dir, "_rn")
cor_list <- list_hiseq_file(dn, "bam_cor_pca_png", "_rn")
cor_list <- purrr::keep(cor_list, file.exists)
if(length(cor_list) > 0) {
  knitr::include_graphics(unlist(cor_list))
}
...
```{r firug6_idr, eval = T, fig.height=5, eval = T}
dn <- list_hiseq_dir(input_dir, "_rn")
qc_dir <- list_hiseq_file(dn, "qc_dir", "_rn")
idr_list <- lapply(qc_dir, function(i) {
  list.files(i, "*idr.txt.png$", full.names = TRUE)
}) %>% 
  unlist()
...
dn <- list_hiseq_dir(input_dir, "_rn")
fp_list <- list_hiseq_file(dn, "bam_fingerprint_png", "_rn")
fp_list <- purrr::keep(fp_list, file.exists)
if(length(fp_list) > 0) {
  knitr::include_graphics(unlist(fp_list))
}
```
...
check_path(data_dir)
# copy files
d1 <- list_hiseq_dir(input_dir, "_r1")
dn <- list_hiseq_dir(input_dir, "_rn")
# report_list <- c()
# for(i in dn) {

report_list <- sapply(sort(c(d1, dn)), function(i) {
  ix <- read_hiseq(i)
