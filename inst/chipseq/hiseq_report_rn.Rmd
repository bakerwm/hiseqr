...
if(is(f6, "character") & length(f6) > 0) {
  if(length(f6)) {
...
f1 <- pd$cor[[1]]$heatmap
f2 <- pd$cor[[1]]$pca
if(is(f1, "character") & length(f1) > 0) {
  if(file.exists(f1)) { 
    knitr::include_graphics(f1)
  }
}

if(is(f2, "character") & length(f2) > 0) {
  if(file.exists(f2)) {
