...
dirs_all <- hiseqr::atac_merge_dirs(input_dir)
if(! is_atac_merge_dir(input_dir)) {
  stop(paste0("Input file is not a ATACseq merge directory: ", input_dir))
}
# dirs_all <- input_dir

# all align
align_list <- sapply(dirs_all, function(f) hiseqr::atac_merge_files(f, "align"))
...
## peak
peak_list <- sapply(dirs_all, function(f) hiseqr::atac_merge_files(f, "peak"))
peak_list <- unlist(peak_list)

## FRiP
frip_list <- sapply(dirs_all, function(f) hiseqr::atac_merge_files(f, "frip"))
frip_list <- unlist(frip_list)

## fragment length
frag_list <- sapply(dirs_all, function(f) hiseqr::atac_merge_files(f, "length_distribution"))
frag_list <- unlist(frag_list)

## count table
count_tab <- hiseqr::atac_merge_files(input_dir, "count_tab")
count_tab <- unlist(count_tab)

## IDR
idr_png   <- hiseqr::atac_merge_files(input_dir, "idr_png")
idr_png   <- unlist(idr_png)
```
...
## total peaks
gr_list  <- lapply(peak_list, function(f) hiseqr::narrowPeakReader(f))
peak_ids <- gsub("_peaks.narrowPeak", "", basename(peak_list))
...
if(file.exists(idr_png)) {
  knitr::include_graphics(idr_png)
}
```
...
```{r fig7_cor_pearson, eval = TRUE, fig.height = 4}
df <- hiseqr::corMatrix(count_tab)
p  <- hiseqr::corPlot(df)
print(p)
```
