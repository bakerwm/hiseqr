...
if(! hiseq_type %in% c("atacseq_rx")) {
  stop(paste0("Input file is not a ATACseq multiple directory: ", input_dir))
}
```
...
# smp merge list
merge_list <- list_atac_merge_dirs(input_dir)
rep_list   <- list_atac_single_dirs(input_dir)
...
```{r}
rpt_html <- hiseqr::get_atac_report(input_dir)
