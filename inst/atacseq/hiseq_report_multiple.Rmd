...
df_trim_stat     <- lapply(merge_list, read_hiseq_trim_stat) %>% bind_rows
df_align_stat    <- lapply(merge_list, read_hiseq_align_stat) %>% bind_rows
df_peak_stat     <- lapply(merge_list, read_hiseq_peak_stat) %>% bind_rows
df_frip_stat     <- lapply(merge_list, read_hiseq_frip_stat) %>% bind_rows
df_lendist_stat  <- lapply(merge_list, read_hiseq_lendist_stat) %>% bind_rows
report_html_list <- sapply(merge_list, list_hiseq_report) %>% unlist %>% unique

# only for merge dir
png_list <- lapply(merge_list, function(i){
  data.frame(
    tss         = read_hiseq(i)$args$tss_enrich_png,
    genebody    = read_hiseq(i)$args$genebody_enrich_png,
    fingerprint = read_hiseq(i)$args$bam_fingerprint
  )
}) %>%
  dplyr::bind_rows()
```
