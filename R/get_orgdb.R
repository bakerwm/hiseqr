...
#' @param format string, url, markdown, html, ...
#'
#' @export
gene_to_link <- function(x, organism, format = "url",
                         force_symbol = FALSE,
  ...
  # check x
  if(force_symbol) {
    x_symbol <- gene_to_symbol(x, organism, "SYMBOL")
  } else {
    x_symbol <- x
  }

  x_ENSEMBL <- gene_to_symbol(x_symbol, organism, "ENSEMBL")
  x_url     <- paste(site, "id", x_ENSEMBL, sep = "/")

  if(format == "markdown") {
    # [label](url)
    paste0("[", names(x_ENSEMBL), "](", x_url, ")")
  } else if(format == "html") {
    # <a href='url'>link text</a>
    paste0("<a href='", x_url, "' target='_blank'>", x_symbol, "</a>")
  } else {
    x_url
  }
