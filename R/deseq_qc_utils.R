...
deseq_qc_dds <- function(x = NULL, ...) {
  ...
  if(is_hiseq_dir(x, "deseq_deseq2")) {
    ...
    res <- run_deseq_res(x, outdir, shrink = FALSE)
  } else {
    warning(glue::glue(
...
deseq_qc_res <- function(x, ...) {
  #-- default values
  shrink_method    <- NULL # standard, normal, apeglm, ashr
  transform_method <- NULL # standard, vst, rlog
  log2fc_limits <- c(-2, 2)
  fc        <- 2
  pvalue    <- 0.05
  p_adjust  <- TRUE
  ...
  } else if(inherits(transform_method, "character")) {
    if(!transform_method %in% c("none", "standard", "vst", "rlog")) {
      warning(glue::glue(
        "transform_method is {transform_method}, expect ",
        "'standard', 'vst', 'rlog'; set to 'none' "
      ))
      transform_method <- "standard"
    }
  } else {
  ...
  #-- shrink
  if(is.null(shrink_method)) {
    shrink_methond <- "standard"
  } else if(inherits(shrink_method, "character")) {
    if(!shrink_method %in% c("none", "normal", "apeglm", "ahsar")) {
      warning(glue::glue(
  ...
  if(inherits(x, "data.frame")) {
    ...
    xn <- list_hiseq_file(x, "norm_fix_csv", hiseq_type = "deseq_deseq2")
    if(inherits(xn, "character")) {
      if(file.exists(xn)) {
        df <- read.csv(xn)
      }
...
deseq_qc_counts <- function(x, outdir = NULL, ...) {
  ...
  if(!inherits(x, "DESeqDataSet")) {
    warning(glue::glue("x is {class(x)}, expect `DESeqDataSet`"))
    return(NULL)
  }
  #-- input
